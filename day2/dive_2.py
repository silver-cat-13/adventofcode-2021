...
# Get the value of to move the submarine
get_value = lambda x: int(x.split()[1])
...
with open('input_data.txt', 'r') as f:
    ...
    while line:
        ...
        if line.startswith('forward'):
            ...
            depth += value*aim
            print(f"Moving forward {value} positions, now in {horizontal_position}")
            print(f"Updating depth based on the aim {aim} and the horizontal_position {value} now " \
                    f"depth={depth}")
        elif line.startswith('down'):
            aim += value
            print(f"Moving down aim {value} positions, now in {aim}")
        elif line.startswith('up'):
            aim -= value
            print(f"Moving up aim {value} positions, now in {aim}")
        line = f.readline()

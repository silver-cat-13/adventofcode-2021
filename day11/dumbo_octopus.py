...
0532250600
0032240000
After step 10, there have been a total of 204 flashes. Fast forwarding, here is the same configuration
every 10 steps:
...
Given the starting energy levels of the dumbo octopuses in your cavern, simulate 100 steps. How
...
class OctopusMap:
    # change this if the map size changes
    MAX_X             = 10
    MAX_Y             = 10
    NUMBER_OF_OCTOPUS = MAX_X*MAX_Y

    def __init__(self, octopus_map):
        self.octopus_map = octopus_map
        self.flash_map   = None
        self.octopus_flash = 0
        self.clear_flash()

    def increase_level(self, x, y):
        if x < 0 or y < 0 or x >= OctopusMap.MAX_X or y >= OctopusMap.MAX_Y:
            # out of bound
            return

        if self.flash_map[y][x] == 1:
            # Octopus has been flashed before, do not increase
            return

        self.octopus_map[y][x] = (self.octopus_map[y][x] + 1) % 10
        if self.octopus_map[y][x] == 0:
            self.flash_map[y][x] = 1
            self.octopus_flash += 1

            # octopus flash, increase level of all adjacents octopus
            adjacents_octopus = ((x-1, y-1), (x, y-1),   (x+1, y-1),
                                 (x-1, y),               (x+1, y  ),
                                 (x-1, y+1), (x, y+1),   (x+1, y+1), )

            for adjacent_octopus in adjacents_octopus:
                # increase level with flash ON to all adjacent octopus
                self.increase_level(adjacent_octopus[0], adjacent_octopus[1])

    def clear_flash(self):
        self.octopus_flash = 0
        self.flash_map   = []
        for _ in range(OctopusMap.MAX_Y):
            self.flash_map.append([0]*OctopusMap.MAX_X)

    def print_map(self):
        for row in self.octopus_map:
            row = [str(r) for r in row]
            print("".join(row))
        print("------------------------------------")
        for row in self.flash_map:
            row = [str(r) for r in row]
            print("".join(row))

octopus_map           = OctopusMap(octopus_map_values)
total_flashes         = 0
steps_for_all_flashes = 0
current_step          = 0
all_octopus_flashes   = False


while current_step < 100 or not all_octopus_flashes:
    for y in range(OctopusMap.MAX_Y):
        for x in range(OctopusMap.MAX_X):
            octopus_map.increase_level(x, y)

    # Get the number of flashes for this step
    current_step_flashes = octopus_map.octopus_flash

    # Check if all octopus flashed
    if current_step_flashes == OctopusMap.NUMBER_OF_OCTOPUS:
        all_octopus_flashes = True
        if not steps_for_all_flashes:
            # Get the step for all octopus flashing only for the first time
            print(f"Steps it took for all octopus to flash: {current_step+1}")

    # increase the total flashes only int he first 100 steps
    if current_step < 100:
        total_flashes += current_step_flashes
        if current_step == 99:
            print(f"Number of flashes after 100 steps: {total_flashes}")

    current_step += 1
    # Clear the flashes
    octopus_map.clear_flash()
